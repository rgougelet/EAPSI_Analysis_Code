...
%     ax.XAxis.TickLabelFormat = '%,d';
    ...
    xplane_beep_offsets = [];
    xplane_heard_beep_offsets = [];
    xplane_missed_beep_offsets = [];
    xplane_heard_onsets = [];
    ...
    for s = 1:length(xplane_markers)
        ...
        if xplane_markers(s)==3000 && xplane_markers(s-1) == 0 % Indicator always changes to 3000 from 0 to indicate a recent beep was heard
            ...
            xplane_heard_beep_offsets = [xplane_heard_beep_offsets s-back_s];
        ...
        if xplane_markers(s)==2000 && xplane_markers(s-1) == 0
            ...
            xplane_beep_offsets = [xplane_beep_offsets s-back_s]; % carried over from heard beeps
            xplane_missed_beep_offsets = [xplane_missed_beep_offsets s-back_s];
    ...
    xplane_fly_with_beep_cond_onsets = setdiff(xplane_1000_onsets, xplane_beep_onsets);
...
%     plot(xplane_fly_with_beep_cond_onsets, xplane_markers(xplane_fly_with_beep_cond_onsets), 'g*')
    ...
    cond_tab = sortrows([[xplane_fly_with_beep_cond_onsets; ones(1,length(xplane_fly_with_beep_cond_onsets))]' ;[xplane_rep_beep_onsets; zeros(1,length(xplane_rep_beep_onsets))]']); % FLY = 1, REP = 0
    ...
    cond_tab = sortrows([[xplane_fly_with_beep_cond_onsets; ones(1,length(xplane_fly_with_beep_cond_onsets))]'... % FLY = 1
